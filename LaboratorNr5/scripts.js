                ...
                if (this.readyState == 4 && this.status == 200) {
                    ...
                    for (let i = 0; i < products.length; i++) {
                        if (products[i].getAttribute("id") == productId) {
                            quantity = parseInt(products[i].getElementsByTagName("quantity")[0].childNodes[0].nodeValue);
                            break;
                        }
                    }
                    ...
                    if (quantity > 0) {
                        ...
                        alert(`The ${name} is not available right now!`);
                        console.log("%c Unavailable product: " + name, "color: red; font-weight: bold;");
                    }
                }
            };
            xhttp.open("GET", "available.xml", true);
            xhttp.send();
        });
    });
